...
export default function SchedulerPage({ settings, remotePosts = [], schedulerStatus = null }) {
  ...
  const queuedPosts = useMemo(
    () => (remotePosts || []).filter((post) => ["draft", "queued", "scheduled"].includes(post.status)),
    [remotePosts]
  );
    ...
    <div className="space-y-6">
      <div className="rounded-2xl border border-white/5 bg-slate-900/40 p-6 shadow-sm">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          ...
          >
            scheduler: {settings.schedulerEnabled ? "เปิดใช้งาน" : "ปิดไว้"}
          </div>
        </div>

        <div className="mt-6 grid gap-4 md:grid-cols-4">
          <div className="rounded-xl border border-white/5 bg-slate-950/40 p-4">
            <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500">scheduled posts</p>
            <p className="mt-2 text-3xl font-bold text-white">{scheduledPosts.length}</p>
          </div>
          <div className="rounded-xl border border-white/5 bg-slate-950/40 p-4">
            <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500">queued posts</p>
            <p className="mt-2 text-3xl font-bold text-white">{queuedPosts.length}</p>
          </div>
          <div className="rounded-xl border border-white/5 bg-slate-950/40 p-4">
            <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500">last run</p>
            <p className="mt-2 text-sm font-semibold text-white">
              {schedulerStatus?.lastRun ? formatDate(schedulerStatus.lastRun) : "ยังไม่มีรอบล่าสุด"}
            </p>
          </div>
          <div className="rounded-xl border border-white/5 bg-slate-950/40 p-4">
            <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500">operational status</p>
            <p className="mt-2 text-sm font-semibold text-white">
              {settings.schedulerEnabled ? "พร้อมตรวจคิวตามรอบเวลา" : "หยุดตรวจคิวชั่วคราว"}
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="rounded-2xl border border-white/5 bg-slate-900/40 shadow-sm">
          ...
          {scheduledPosts.length === 0 ? (
            <div className="p-6 text-sm text-slate-400">
              ตอนนี้ยังไม่มีโพสต์ที่ตั้งเวลาไว้ เมื่อมีรายการ scheduled จะขึ้นที่หน้านี้อัตโนมัติ
            </div>
          ) : (
        ...
        <div className="space-y-6">
          <div className="rounded-2xl border border-cyan-500/10 bg-cyan-500/5 p-6 shadow-sm">
            ...
            <p className="mt-3 text-xs leading-relaxed text-slate-300">
              หน้านี้ยังใช้ scheduler V1 ตัวเดิมที่เสถียร ไม่มี Queue V2 และไม่มีการเปลี่ยน engine ใต้ระบบ
            </p>
            {schedulerStatus ? (
              <div className="mt-4 rounded-xl border border-white/5 bg-slate-950/40 p-3 text-[11px] text-slate-300">
                รอบล่าสุด: สำเร็จ {schedulerStatus.published || 0} • ไม่สำเร็จ {schedulerStatus.failed || 0} • ครบเวลา {schedulerStatus.due || 0}
              </div>
            ) : null}
          </div>
          ...
          <div className="rounded-2xl border border-white/5 bg-slate-900/40 p-6 shadow-sm">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-semibold uppercase tracking-wider text-white">หมายเหตุการใช้งาน</h3>
            </div>
